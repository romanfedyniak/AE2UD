...
public class AEGuiHandler implements IAdvancedGuiHandler<AEBaseGui>, IGhostIngredientHandler<AEBaseGui>, ISlotIngredientProvider<AEBaseGui> {
    ...
    /**
     * Turns a key into the ingredient HEI knows how to look recipes up for, through whatever
     * {@link IngredientConverters converter} speaks for its type - a {@code FluidStack} for a fluid.
     * <p>
     * A key type nothing is registered for falls back to the placeholder stack it wraps itself in, which is
     * what HEI would have read off the slot anyway.
     *
     * @param amount only matters for a type HEI measures, and only in that it must not be zero, since an
     *               empty ingredient is one HEI ignores. Terminal rows ask with amount 0 because their
     *               amount is drawn separately, and a recipe lookup does not care either way.
     */
    @Nullable
    private static Object asIngredient(@Nullable AEKey what, long amount) {
        ...
        final Object ingredient = IngredientConverters.toIngredient(what, amount);
        return ingredient != null ? ingredient : what.wrapForDisplayOrFilter();
    }
    ...
    public static AEKey keyOf(final Object ingredient) {
        final GenericStack stack = IngredientConverters.toStack(ingredient);
