...
- Icons drawn from the mod's own sheet came up as solid black squares whenever a filled rectangle had been drawn just before them: the marker showing that a crafting CPU is kept for players or for automation, on a CPU far enough into a job to have a progress bar, and every source icon in the crafting plan tree - taken from storage, promised by a level emitter, or missing. Those icons are mostly empty field around a small glyph, and Minecraft's rectangle drawing leaves blending switched off behind it, so the field was filled in black instead of showing what was underneath. The same left a row plate in the CPU list drawing its bottom row - the gap between rows - as a dark line under any row following one with a progress bar. Each of them now sets up blending itself rather than inheriting whatever the last thing drawn happened to leave.
