...
- **The terminal switches mode from inside its own screen.** A button at the foot of the column down the left of every wireless terminal carries the mode it is in; clicking it opens the rest out to the left, and clicking one of those changes to it without closing the terminal. Modes that have not been unlocked are in there too, greyed, with a tooltip naming what to craft in to get them - a terminal with one mode would otherwise show a single button and nothing on the screen would say that modes exist at all. The fan is reported to HEI while it is open and the one button while it is shut, so the item list never covers a mode you are reaching for.
