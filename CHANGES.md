...
- **A formation plane can be told which key types it places.** It builds one placement strategy per registered type and hands each key to whichever answers, so a plane with an empty filter accepted anything the network cared to push at it - there was no way to say "blocks, but never fluids" short of naming every block in the filter. The button is the same picker, and a type turned off is left out when the strategies are built, so the plane simply reports back that it took nothing. Its window says "Placed Types" rather than borrowing the words meant for a bus.
