...
- **What a storage cell holds is kept in the world's `AE2/cells` folder, not in the cell item.** Every cell a world touches moves there. Going back to an older version shows those cells empty, a backup or copy of the world without that folder loses them, and a cell item taken to another world or server arrives empty.
...
- **An addon's storage cell comes apart in a crafting grid.** An empty cell placed alone in the grid gives back
  ...
  on `AbstractStorageCell`, whose component is reachable through `getComponent()`.
...
- **An internal placeholder item was listed in HEI**, showing its own translation key as its name. The item exists only to carry a fluid or another non-item key where an `ItemStack` is demanded; it is on no creative tab, refuses to appear in creative search and deletes itself out of a player's inventory. But an item on no tab answers Forge's "which tabs are you on" with a list holding one nothing, and HEI reads that as leave to list the bare item without asking the item what it has to offer - which is nothing. It answers with an empty list now. It also has a name and a picture at last - a blank pattern - for wherever else a placeholder could be made to surface: a placeholder carrying nothing used to draw nothing at all, which reads as a missing model rather than as what it is.

- **Two lines in every log claimed this mod was broken.** The item and fluid key types are registered while Forge's new-registry event runs, and that event belongs to no mod in particular - so FML read the names `appliedenergistics2:item` and `appliedenergistics2:fluid` as an attempt to override something of Minecraft's and warned about a "Potentially Dangerous alternative prefix", which "in most cases indicates a broken mod". The names were always kept as given and nothing was ever wrong; the mod simply says who it is while those two names are set now, and the warnings are gone.
...
- **Thirty-seven strings nothing asked for any more are gone**, from every language: the titles of the fluid bus, plane, emitter and terminal windows deleted when those parts were folded into the ordinary ones, tooltips for buttons that no longer exist, and the names of two items that no longer exist either. The constants behind them went too, along with six that had never had a string at all, and the translations that no longer matched any key English still has - mostly left behind by renames, such as the Interface Terminal becoming the Interface Configuration Terminal.

- Five debug blocks - the item generator, chunk loader, phantom node, cube generator and energy generator - had no name, so they showed their translation key and could not be searched for. They are named like the other developer tools now, `Dev.ItemGen` and so on.

- **An upgrade card's tooltip filed the portable cell and the Colour Applicator under "ME Storage Cells".** A card lists where it fits by group, and both tools gave the storage cells' group as their own, so the Energy Card - which no storage cell takes - claimed to fit one, and the applicator vanished into the same line on every card they share. A portable cell is listed under "Portable Cells" now, and the applicator under its own name.

- **A fluid amount on the crafting tree, the crafting plan, the CPU screen or a monitor was printed digit by digit whenever it was not a whole number of buckets.** Twelve million buckets and change read `12315561B` instead of `12MB`. The shared formatter has a branch for an amount that does not divide evenly into its unit, and that branch never looked at which form it had been asked for: it always printed the full count of buckets. The same branch is what a tooltip goes through, so an exact reading lost its digit separators too, printing `12315561.2B` where it means `12,315,561.2B`. It answers in the form it was asked for now - shortened for a screen, grouped for a tooltip - and the fractional digit is kept only while the number is short enough for it to mean anything, so a bucket and a half still reads `1.5B`.

- **A shortened amount stopped at the wrong suffix, and rounded up when it got there.** Six billion items read `6000M` rather than `6G`, seven and a quarter billion buckets read `7241MB` rather than `7.2GB`, nine million nine hundred and ninety-nine thousand nine hundred and ninety-nine items read `10M`, and one short of a million read `1000K`. One loop decided all of it, and it was wrong twice. It stopped as soon as the number fitted the width the form allows - four digits - so a bigger suffix that was sitting right there was never reached; the number stays under a thousand once any suffix is in play now, and the form's own width decides only whether to shorten at all. And it used the default rounding, which rounds up, so a reading could claim a whole magnitude that is not there; it rounds down now, the way the slot form already did and the way this file's own comment already said it should. The four readings above are `6G`, `7.2GB`, `9.9M` and `999K`.

- **A round amount printed a fractional zero it did not need.** A trillion of something read `1.0T` in a slot, where every other reading of the same number said `1T` - and the zero was spending one of the four characters a slot has to give. The pattern behind the slot form demanded a fractional digit whether or not there was one; its only other trick, writing `.5` for a half, could never fire, because that form is only reached once the number is too wide to print whole. It is the same form the rest of the file uses now, and there is one of it rather than two.

- **"Steps" and "To Craft" read the same number two different ways.** They sit one under the other on the crafting plan, and a job of a trillion showed `To Craft: 1T` above `Steps: 1.0T`: the first line used the form meant for a line of text, the second the form meant for a 16x16 slot. Their tooltips disagreed too - one grouped the digits by the mod's own rule and the other by whatever the machine's language settings said, so the two lines could be separated by a comma on one computer and a full stop on the next. Both lines and both tooltips go through one formatter now.
...
- **An upgrade card shift-clicked into an interface goes into the upgrade slots or nowhere.** It used to end up in the two rows in the middle of the screen whenever the four slots on the right would not take it - a card the interface has no use for, one over its limit, or simply the rest of the stack after the first went in. Those rows are what the interface stocks and hands to the machine beside it, so a card put there quietly walked into a furnace. Shift-clicking a card now fills the upgrade slots and leaves whatever will not fit in your own hands, which is also how it behaves out of a network tool. Putting a card in those rows on purpose still works - drag it in, as before.
...
- **Every monitor has a window now.** Sneak-right-click a Storage or Conversion Monitor with an empty hand and it opens: a slot for what it watches, which takes an item from the inventory below or one dragged out of HEI, and holds a fluid or any other kind of key just as well; and the lock, which used to be that same gesture. Nothing else changed - clicking a monitor with something in hand still points it at that, an empty hand still clears it, a wrench still turns the display, and a locked monitor still keeps its key against all of them, and against a drop from HEI. The lock moved because there was one gesture per setting and none left over for a third.

- **A monitor can show how fast what it watches is moving.** Two buttons in that window: the span - `Off`, `/t`, `/s`, `/m`, `/h` - and which figure to show - `Net`, `In`, `Out`. Switched on, a coloured line appears under the amount on the monitor's face: green when the network is gaining, red when it is losing, grey when nothing is moving. On a locked monitor the lock's two lower corner marks make way for that line, and the upper two still show the lock. **Net** is what the network gained or lost, which nothing can inflate; **In** and **Out** are what actually passed through, which is the only way to see a line that produces and consumes at the same rate - net calls that zero. Waila shows all three at once. The span also decides how much history the figure is averaged over, five seconds for the short units and a minute for the long ones, so a per-tick reading is allowed to jump while an hourly one holds steady; `/h` measures a minute and multiplies rather than waiting an hour to say anything. The idea is GregTech: New Horizons' Throughput Monitor, which Advanced AE also has; both are separate parts to craft and both measure only the net change. This is a mode on the monitors already on the wall, off until switched on.

- **It costs the server nothing until someone switches it on.** The network already sees every change to its contents one at a time; metering keeps the two directions apart instead of adding them up, and only for the handful of keys a monitor actually asked about - one lookup per change, behind a test that is free while nobody is metering. It is deliberately not something a watcher gets for free: a terminal watches every key in the network, and a meter inherited from that would meter the lot. A metering monitor sends its rate to nearby players only when the number a player would read changes, rather than every tick it moves. `monitorThroughput` in the config turns the feature off for a server entirely.

### Network visualiser

- **A new tool draws the network in the world.** Right-click a machine or a cable with the Network Visualiser and it draws the whole network around it: a cube at every block with a node, a box along every connection, and, close up, the number of channels each connection carries. A link is coloured by how full it is and drawn as thick as it can carry - the thickness comes from the channel tier, so a tier an addon registers is drawn wider with no code here - a node turns red when something at that position is asking for a channel it did not get, and a P2P link wears the four colours of its frequency, the same four the tunnel face shows, so several tunnels in one block can be told apart. It binds to the *face* that was clicked, which means clicking the face of a P2P tunnel shows the sub-network behind it. Right-clicking the air steps through four modes and sneak-right-clicking unbinds. Binding needs `BUILD` on that network and the check is repeated every time the picture is rebuilt, so it cannot be used to read a stranger's base through its walls. The idea comes from AE2Stuff by bdew and from the port of it in GregTech: New Horizons; the code is this fork's, since AE2Stuff is under a licence that cannot be mixed into an LGPL tree.

- **It is built so that neither side pays for it when it is not in use.** A network nobody is pointing a visualiser at costs nothing at all - no grid cache, no field, no per-tick work. A held one is served at the end of the server tick: the network is walked at most once every twenty ticks by default, **one walk serves everyone watching it**, and the result is not sent at all if it has not changed. The walk follows connections outwards from the bound block and stops at 16 384 of them, so what is drawn is always a connected picture around what you bound rather than scattered fragments; both numbers are in the config. On the client the geometry is world-space boxes in one buffer on the card, rebuilt only when the network or the mode changes, and drawn in a single call - AE2Stuff rebuilds a quarter of a million vertices through the tessellator sixty times a second, which is why it costs frames. Its lines also use `glLineWidth`, which is measured in screen pixels, so a receding network keeps four-pixel lines while everything between them shrinks and turns into a solid mat; a box is measured in blocks and thins away like the cable it follows. The channel numbers are laid out from the font sheet by hand, since the alphabet is ten digits, so every number on screen is two draw calls rather than one call each.

- **The colours are all in the client config**, along with the render distance, the distance at which numbers appear, and the size of a node and a link. The load gradient defaults to green and red, which is the pair the commonest colour blindness cannot separate, so both ends of it can be changed.

- **For addon authors:** anything that makes a real `IGridConnection` is already drawn - the visualiser walks connections, so a wireless connector joining two distant nodes appears as a long link with no code at all. `INetworkVisualiserProvider` is for the rest: giving your links and blocks a look of their own, and adding machinery that belongs in the picture but has no node in the grid. A style crosses the wire as a `ResourceLocation` and becomes a colour on the client through `NetworkVisualiserStyles`, so the colour stays the player's to change and an addon installed only on the server leaves links that are drawn plainly instead of a client that cannot read the packet. This is api amendment 51; the GregTech: New Horizons port of AE2Stuff, which had to import GregTech's own tile entities into the visualiser item to draw its proxy hatches, is why it exists.
...
- **Particles near cables stop costing the client a tenth of its time.** Anything that moves with collision on - and a Thaumcraft particle does, thousands of them at once around a large base - asks every block it touches for its shape each tick, and a cable bus built that shape from scratch every time. Worse, it looked up the cable render mode once per side while doing it, which walks the player's hotbar looking for a network tool, so one cable cost 63 slot checks per particle per tick - for an answer a collision box never uses, since only the selection outline hides facades. Measured on a client profile of such a base at **2.6 seconds of 23**, five times what the EnderIO conduits in the same query cost. The client now keeps each bus's collision shape until something changes it: a packet for that bus or for one beside it (a dense cable and a plane both take their shape from their neighbour), a part added or removed, the bus turning. The render mode is read only for the outline, on both sides. The server builds its shapes as before - it has no particles, and it never had the hotbar walk.
...
- **A client uses the channel tiers of the server it is on.** A tier's number comes from the config file, and nothing made a client's file agree with the server's, so a pack that changed a tier only on the server had cable tooltips, the channel marks on smart cables and Waila showing the client's numbers while the network ran on the server's. The server sends its numbers as a player joins, and the client goes back to its own once it leaves.
...
- The One Probe lists an annihilation plane's enchantments, under the same "Enchanted with:" heading Waila already showed. They decide what the plane does - silk touch and fortune change what it gets back, efficiency and unbreaking what it costs in power - and a placed plane looks the same with or without them, so with The One Probe the only way to find out was to break the plane and read the item.

### Storage cells

- **A storage cell keeps what it holds in a file of its own, not in its item.** A cell's contents lived in the item's NBT, which grew with every type it held and every tag those types carried, went into the chunk or the player file with it, and was read back whole whenever anything opened the cell: a drive of full cells could push a chunk past what the game can save, and a portable cell reloaded its entire contents for every block a mining tool broke. The contents live on the server now, one compressed file per cell under the world's `AE2/cells`, named by an id the item carries, and every inventory opened on the same cell shares one copy in memory. The item keeps a summary - the id, its type and byte counts and the five entries there is most of - so a tooltip still reads without asking the server; the view key's window asks for the whole list when it opens, and says so until it arrives. Files are written when the world saves, on a thread of their own, and `/save-off` holds them back like everything else. A cell is given its id by the first thing put into it and loses it, and its file, when it is emptied. A cell that already holds something moves to a file the first time the server opens it, and one found with contents both in its item and in a file has the two added together.
- **Putting an item into a cell no longer builds a copy of it first.** A cell makes sure it is not being handed a storage cell with something still in it, and it found out by building an item stack - capabilities, tag and all - out of every item a network put into it. It asks the item now, and builds a stack only for an item that really is a cell. An addon's cell handler can answer the same way through `ICellHandler.isCell(AEItemKey)`, registered as amendment 56; one that does not is asked as it was before.
- **A storage cell adds up what changed instead of adding up everything.** Every insertion and extraction recounted the cell's whole contents to work out its new total, and threw away its per-type byte counts so that they had to be rebuilt the next time anything asked how full it was. On a full cell that was a pass over sixty-three entries per item moved, twice; it is now one addition. Measured at 1 100 ns down to 106 ns per insertion into a full cell held in a drive.
...
- **Portable cells go up to 16384k as well.** The Portable Cell is the 1k tier now, and seven follow it - 4k, 16k, 64k, 256k, 1024k, 4096k and 16384k - each crafted the way the first always was, from an ME Chest, the storage component of its tier and an Energy Cell. A portable cell holds half of what a storage cell of its tier holds, which is what the 1k one already did: 512 bytes up to 8,388,608, with a type costing 8 bytes up to 131,072. Everything else is the same on every tier - 27 types, the battery, the charge rate and the four card slots - so a bigger tier holds more, not differently. The tiers above 64k are behind the same `HighCapacityStorage` switch as the cells, and any of them earns the Storage Nomad advancement. Each is drawn as the storage cell of its tier with the portable cell's screen over it, which hides the colour square a storage cell shows its tier with, so the tier's colour is a light of its own on the cell's right arm, one pixel up from the green one; the 1k cell gains that light too.

- **Portable fluid cells**, in the same eight tiers, 1k to 16384k. Each is crafted from what the item one of its tier is, with a Fluid Cell Housing added - the same housing that tells a fluid storage cell from an item cell. It holds half of what a fluid storage cell of its tier holds and five types, as a fluid storage cell does, and takes the item portable cell's cards except the fuzzy card, which no fluid cell is offered, so it has three card slots rather than four. The battery, the charge rate and the energy cards are the item portable cell's, and any of them earns the Storage Nomad advancement. Each is drawn as the fluid storage cell of its tier under the same screen and tier light. Upstream AE2 has portable fluid cells up to 256k.

- **An item portable cell can pick things up.** Shift and right-click one to switch auto pickup on, and what the player walks over - and what the blocks they break drop, which then never reach the ground - goes into the cell instead of the inventory. It takes whatever the cell would accept in a drive: a partitioned cell only what it is partitioned to, an inverted one everything else, an unpartitioned one anything, with the fuzzy, equal distribution and overflow destruction cards applying as they would there. That last one means a cell carrying overflow destruction destroys what no longer fits instead of letting it through to the inventory. What does not fit goes to the next such cell and then to the inventory; cells partitioned to the thing are offered it first, and otherwise the hotbar comes before the inventory and the inventory before the offhand. Each item costs the cell 1 AE, as putting it in through the cell's window does, so a flat battery picks nothing up, and a cell whose window is open goes on taking things straight into that window. A tool that breaks a whole area at once finds the player's cells once for the lot rather than once per block. Items someone else dropped, drops a block has only a chance of giving, and anything a fake player breaks are left alone. The state is on the tooltip and is shown above the hotbar when it is switched. The idea comes from the dense portable cells of [NAE2](https://github.com/AE2-UEL/NAE2); unlike those, part of a stack goes in when the whole will not fit, and opening a cell does not switch pickup off.
...
- **The crafting terminal has a second arrow, for emptying the grid into your own inventory.** It had one button, which returned what was standing in the grid to network storage; the other half of the gesture - keep these items, I am about to walk off with them - meant taking all nine stacks out by hand. There are two buttons now, side by side to the right of the grid: the left one points up and sends the grid to the network, the right one points down and sends it to your inventory, which is where each of those things is drawn on the screen. Anything your inventory has no room for stays in the grid rather than being dropped or quietly rerouted, so the other button is still there to take the rest. Upstream AE2 has the same pair. Both terminals get it, wired and wireless: they now share one screen class instead of two identical copies, which is why they cannot drift apart again. The two arrows are new drawings - the old button wore a shape it shared with the "clear settings" and "reset name" buttons elsewhere in the mod, and a shared drawing cannot be changed for one of them.

- **A tank takes a tankful, not a bucketful.** Clicking a fluid on a terminal moved exactly one bucket, whatever was in hand, so filling a mod's multi-bucket cell meant clicking it once per bucket and emptying one meant the same again. A click now moves as much as the container itself takes or gives: a bucket still takes a bucketful, because that is all a bucket holds, and a cell that holds sixty-four takes sixty-four. The same limit came out of the other two places the gesture appears - a filled container shift-right-clicked out of your own inventory, and an interface's fluid slot clicked directly - so the three behave alike rather than one of them being the odd one out. The conversion monitor in this fork already worked this way, which is what made the terminal the exception; GTNewHorizons' fork fills by capacity too. Where a whole stack of containers is filled or emptied at once with ctrl, the stack comes out even: rather than fill the last one part-way, the network keeps the remainder, since containers processed in one click are handed back as one stack and a part-filled odd one out cannot be represented in it.
...
- **A ghost dragged out of HEI says which button does what.** The lines that tell a filter slot's two clicks apart are built from what the cursor is carrying, and a drag out of a recipe viewer carries nothing at all - so the one drop where the two buttons differ, a bucket over a row that can hold a fluid, explained itself to nobody. While HEI drags an ingredient, a filter slot under it now says that the left button sets it to what the container holds and the right to the container itself. Only for a container whose contents that slot would take: everything else lands the same whichever button ends the drag, and a hint for a difference that is not there is worse than none. The hint appears on the end of the hovered item's tooltip, or on its own over an empty slot, since the hand is empty either way.
...
- **A mode comes back out of the terminal the way it went in.** Put the terminal alone in a crafting grid and the terminal part of the mode it is showing comes out, while the terminal stays in the grid without that mode - same charge, same link, same cards, every other mode untouched. To take out a different mode, switch to it first. The last mode never comes out: a terminal with nothing written on it reads as a plain storage terminal, so taking its only mode would hand over a part and leave that mode behind for nothing. What the mode kept on the terminal - the crafting grid, or the blank and encoded patterns - goes to the player's inventory, and to their feet when that is full. Crafted by a machine, there is nobody to give it to, so it stays on the terminal and is there again when the mode is put back. The terminal's tooltip says which mode a craft would take out, and HEI shows one entry per mode.
...
- **The list of places to send a pattern puts the ones that can take it first.** A full interface used to sit among the ones that could take the pattern, looking like an answer and refusing the click; now the list reads as a slope - takes it now, would take it if you freed a slot, will never take it - with names in order inside each group. A full one sorts above an unsuitable one on purpose: freeing a slot is something you can go and do, and for a crafting pattern the unsuitable ones are most of a large network, so a full bank of assemblers would otherwise be buried under all of them.

- **The pattern terminal's send button takes the last pattern back.** Sending a pattern away is one click, and until now realising it went to the wrong interface meant walking there. With nothing in the encoded slot, the same button turns into a downward arrow and returns the last pattern you sent - out of wherever it went and back into the slot it left. It offers this until you use it, send another pattern, or log out, and it checks at the moment you press it: the pattern comes back only if that machine is still on this terminal's network and still holds it, and says which of those failed otherwise. Taken from the same pair of addons the sending itself came from, AE2 Auto Pattern Upload and Pattern Uploader, which put it on a modifier key; here the button has nothing else to mean while the slot is empty, so it needs none.
...
- An addon can build against AE2UD without a release and without building AE2UD first. [JitPack](https://jitpack.io) builds whichever pushed commit or tag is asked for, as `com.github.romanfedyniak:AE2UD:<commit or tag>`, and `jitpack.yml` tells it to run Gradle on Java 21 and to publish under those coordinates.
- What AE2UD publishes no longer says it needs GregTech CE Unofficial, IndustrialCraft 2, Forestry, CodeChicken Lib, Baubles, The One Probe and HEI to run. It works with each of them and needs none; only MixinBooter is listed now, so an addon built against AE2UD does not pull all seven into its own development client. AE2UD's own development client still carries them.
